...
// console.log(toNumber(response.resultInfo) ===1)
    return (
      <>
        <div className="searchGrid">
          <div className="row m-0 login-firstbar" id="loginDiv">
            ...
            <div className="col-sm-12 px-4 inpt-p">
              <div className={`row height-50 inpt-i ${response.results && response.resultInfo && response.resultInfo.hits >= 0 ?"spacingcss":""}`}>
                <div role="search" className={`topbar_search inpt-li${!parameters.query && !this.state.isLoading?"searchNoParams":""}`}>
          ...
          <div id="result-facet-container" style={{backgroundColor: response.results !== undefined?"#fff":""}} role="main">
            {
                  ...
                  <div  id="results-container-main" className={(!defaults.facetsFiltersDisplay || response.facets === undefined || response.facets.length <= 0 || response.results === undefined || parseInt(response.resultInfo.hits, 10) === 0)?'resultNoFaacets':'resultWithFacet'}>
                    
                    {
                      ...
                      <div className="display-count-data">
                        <nav className="count-data-nav">
                            <span className="resultsText">Page {response.resultInfo.currentPage} of {response.resultInfo.lastPage}</span>
                              <span className="next-prev-btn">
                                <a href='javascript:;'className={`${response.resultInfo.currentPage === 1? "inactive":"active" }`} onClick={(e)=>{this.getPage(e,response.resultInfo.currentPage - 1)}}>
                                <span className={`left-trans-arrow `}>←</span>
                                </a>
                                <a href='javascript:;' className={`${response.resultInfo.currentPage === response.resultInfo.lastPage ? "inactive":"active"}`} onClick={(e)=>{this.getPage(e,response.resultInfo.currentPage + 1)}}>
                                <span className={`right-trans-arrow ${response.resultInfo.currentPage !== 1? "active-right":""}`}>→</span>
                                </a>
                              </span>
                                </nav>
                            </div>
